...
var (
	...
	allFilters = []*Filter{filter1, filter2, filter3, filter4, filter5, filter6, filter7, filter8, filter9, filter10, filter11, filter12, filter13, filter14}
)
...
var announces = []struct {
...
}{
	{
		...
		[]*Filter{filter4, filter6, filter12},
	},
	{
...
func TestRegexp(t *testing.T) {
	...
	for _, announced := range announces {
		...
		if announced.expectedHit {
			...
			satisfied := 0
			fmt.Println("Testing filters with: " + release.ShortString())
